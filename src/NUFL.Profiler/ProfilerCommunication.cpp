...
#undef RELTRACE
#define RELTRACE(x) //wprintf_s(x);wprintf_s(_T("\n"));
...
ProfilerCommunication::~ProfilerCommunication()
{
	//this->m_eventThreadTerminate.Set();
	//this->m_eventThreadTerminated.Wait();
}
...
bool ProfilerCommunication::Initialise(TCHAR *key, TCHAR *ns)
{
	...
	if (AllocateBuffer(MAX_MSG_SIZE, bufferId))
	{
		...
		if (!InitializeFlusher(memoryKey))
		{
			return false;
		}
    }
...
bool ProfilerCommunication::InitializeFlusher(std::wstring &memoryKey)
{
	//these two events below are used for flushed

	m_eventHostRequireFlush.Initialise((m_namespace + _T("\\OpenCover_Profiler_Communication_RequireFlush_Event_") + memoryKey).c_str());
	if (!m_eventHostRequireFlush.IsValid()) return false;
	hEvents[0] = m_eventHostRequireFlush.m_hEvent;

	m_eventProfilerFlushed.Initialise((m_namespace + _T("\\OpenCover_Profiler_Communication_ProfilerFlushed_Event_") + memoryKey).c_str());
	if (!m_eventProfilerFlushed.IsValid()) return false;

	m_eventThreadTerminate.Initialise(NULL);
	if (!m_eventThreadTerminate.IsValid()) return false;
	hEvents[1] = m_eventThreadTerminate.m_hEvent;

	m_eventThreadTerminated.Initialise(NULL);
	if (!m_eventThreadTerminated.IsValid()) return false;
	...
	HANDLE handle =  CreateThread(
		...
		0, //DWORD cbStack,
		FlusherThreadFunc, //PTHREAD_START_ROUTINE pfnStartAddr,
		this, //PVOID pvParam,
		0, //DWORD fdwCreate,
		&dwThreadID //PDWORD pdwThreadID
		);
	if (handle <= 0)
	{
		return false;
...
DWORD WINAPI ProfilerCommunication::FlusherThreadFunc(PVOID pvParam)
{
	RELTRACE(_T("Flusher thread started."));
	ProfilerCommunication* pProfilerCom = (ProfilerCommunication*)pvParam;
	return 0;
	while (true)
	{
		DWORD dw = WaitForMultipleObjects(2, pProfilerCom->hEvents, FALSE, 5000);
		switch (dw)
		{
		case 0:
			//TO DO: there's potential race condition here
			pProfilerCom->m_eventHostRequireFlush.Reset();
			pProfilerCom->SendVisitPoints();
			pProfilerCom->m_eventProfilerFlushed.Set();
			RELTRACE(_T("Flusher thread flushed."));
			break;
		case 1:
			pProfilerCom->m_eventThreadTerminate.Reset();
			pProfilerCom->m_eventThreadTerminated.Set();
			RELTRACE(_T("Flusher thread exited."));
			return 0;
			break;
		default:
			RELTRACE(_T("Flusher thread error exited."));
			return -1;
		}
	}
...
{
	RELTRACE(_T("Profiler: GetSequencePoints."));
    if (!hostCommunicationActive) return false;

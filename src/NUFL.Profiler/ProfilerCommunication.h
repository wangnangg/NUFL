...
private:
	bool InitializeFlusher(std::wstring &memoryKey);
	HANDLE hEvents[2];
	static DWORD WINAPI FlusherThreadFunc(PVOID pvParam);
	CEvent m_eventHostRequireFlush;
	CEvent m_eventProfilerFlushed;
	CEvent m_eventThreadTerminate;
	CEvent m_eventThreadTerminated;
